...
4. 샤프트 끝면이나 힌지 기준면처럼 축에 수직인 평면이 있다면 `3D 평면에서 중심축 선택`을 누르고 3D 화면에서 그 면을 클릭합니다. 테셀레이션된 동일 평면 전체의 중심이 관절 원점으로, 면 노멀이 관절축으로 입력됩니다. 노멀 방향이 반대라면 `방향 반전`을 누릅니다. `Esc`로 면 선택을 취소할 수 있습니다.
5. 적절한 평면이 없다면 필요한 형상을 선택하고 `선택 형상 중심 사용`을 누르거나 `원점 (mm)`과 축을 직접 수정합니다. 회전관절에서는 이 원점이 회전축이 통과하는 중심점이므로 힌지, 샤프트 또는 베어링 중심과 맞춰야 합니다.
6. 값을 바꿨다면 `동작 설정 적용`을 누릅니다.
7. `간격`을 정한 뒤 `−`/`+`를 누르면 해당 간격만큼 이동합니다. 기본 간격은 직선관절에서 5 mm입니다.
...
3D에 표시되는 주황색 양방향 선은 실제 회전축이며 관절 원점을 통과합니다. 평면 선택은 평면 중심과 노멀을 계산하지만 원통면이나 베어링의 중심축 자체를 자동 판별하는 기능은 아니므로, 적절한 기준 평면을 선택한 뒤 슬라이더로 회전 중심을 확인해야 합니다.
...
- [Windows x64 다운로드 (.zip)](https://github.com/kraizer-ai/step-urdf-maker/releases/download/v0.1.4/step-urdf-maker-0.1.4-windows-x64.zip)
- [Linux x64 다운로드 (.tar.gz)](https://github.com/kraizer-ai/step-urdf-maker/releases/download/v0.1.4/step-urdf-maker-0.1.4-linux-x64.tar.gz)
- [Linux ARM64 다운로드 (.tar.gz)](https://github.com/kraizer-ai/step-urdf-maker/releases/download/v0.1.4/step-urdf-maker-0.1.4-linux-arm64.tar.gz)
