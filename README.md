...
- 관절 자동 왕복 `자동 Play`와 핸들·주행 레버만 표시하는 `조작 Play`
...
- `🎮 조작`: 설정용 좌우 패널을 숨기고 3D 화면 안에 실제 조작 입력만 표시합니다. mimic의 원본 관절은 `연동 입력`, 바퀴 속도 구동 원본은 `주행 레버` 슬라이더로 자동 추출됩니다. 현재 구성에서는 핸들과 전·후진/가속 레버만 나타납니다.
