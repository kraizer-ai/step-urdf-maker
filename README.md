...
4. 움직일 자식 형상을 선택하고 `+ 선택 형상으로 자식 링크`를 누르면 부모와 자식만 확대됩니다. 관절 중심의 시작값은 자식 형상 묶음의 BBox 중심입니다.
5. 선택한 자식 메시에서 `A 장축`, `B 중간축`, `C 두께/노멀` 후보를 계산해 3D에 표시합니다. `revolute`/`continuous`를 고르면 평면형 핸들의 회전축으로 쓰기 좋은 C축을 우선 추천하고, `prismatic`은 A축을 추천합니다.
6. 생성 창의 `동작 미리보기` 슬라이더를 움직여 자식이 실제로 중심축 주위를 도는지 확인합니다. 중심이 어긋나면 같은 창의 `관절 중심 (mm)`을 수정합니다. 임의 방향은 `±X`, `±Y`, `±Z`나 축 숫자로 직접 지정할 수 있습니다.
7. 기존 관절의 중심을 수정할 때는 필요한 형상을 선택하고 `선택 형상 중심 사용`을 누르거나 `원점 (mm)`과 축을 직접 수정합니다.
8. 값을 바꿨다면 `동작 설정 적용`을 누릅니다. `간격`을 정한 뒤 `−`/`+`를 누르면 해당 간격만큼 이동합니다.
...
3D에 표시되는 주황색 양방향 선은 실제 회전축이며 관절 원점을 통과합니다. 새 링크의 A/B/C 후보축은 선택한 자식 형상의 분포에서 구한 주축이고, 그중 C는 가장 얇은 방향 또는 평면 노멀입니다. 자동값은 시작점이므로 생성 창의 슬라이더로 회전 중심을 확인하고 필요한 경우 원점을 직접 미세 조정합니다.
...
- [Windows x64 다운로드 (.zip)](https://github.com/kraizer-ai/step-urdf-maker/releases/download/v0.1.6/step-urdf-maker-0.1.6-windows-x64.zip)
- [Linux x64 다운로드 (.tar.gz)](https://github.com/kraizer-ai/step-urdf-maker/releases/download/v0.1.6/step-urdf-maker-0.1.6-linux-x64.tar.gz)
- [Linux ARM64 다운로드 (.tar.gz)](https://github.com/kraizer-ai/step-urdf-maker/releases/download/v0.1.6/step-urdf-maker-0.1.6-linux-arm64.tar.gz)
