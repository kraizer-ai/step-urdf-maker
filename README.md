...
4. 자식 링크를 새로 만들면 부모 링크가 3D 화면에 확대되고, 선택한 자식 형상 묶음의 BBox 중심을 통과하는 X/Y/Z 후보축이 표시됩니다. 선택 형상이 없으면 부모 링크의 BBox 중심을 사용합니다.
5. 생성 창에서 3D와 같은 색의 `±X`(빨강), `±Y`(초록), `±Z`(파랑) 버튼 중 하나를 선택합니다. 축의 선 자체는 양방향으로 같고 `+`/`−`는 양의 회전 또는 이동 방향을 정합니다.
6. 기존 관절의 중심을 수정할 때는 필요한 형상을 선택하고 `선택 형상 중심 사용`을 누르거나 `원점 (mm)`과 축을 직접 수정합니다.
7. 값을 바꿨다면 `동작 설정 적용`을 누릅니다.
8. `간격`을 정한 뒤 `−`/`+`를 누르면 해당 간격만큼 이동합니다. 기본 간격은 직선관절에서 5 mm입니다.
...
3D에 표시되는 주황색 양방향 선은 실제 회전축이며 관절 원점을 통과합니다. 후보축은 선택 형상 BBox 중심과 부모 링크 좌표계의 X/Y/Z를 기준으로 하므로 생성 후 슬라이더로 회전 중심을 확인하고, 필요한 경우 원점을 직접 미세 조정합니다.
...
- [Windows x64 다운로드 (.zip)](https://github.com/kraizer-ai/step-urdf-maker/releases/download/v0.1.5/step-urdf-maker-0.1.5-windows-x64.zip)
- [Linux x64 다운로드 (.tar.gz)](https://github.com/kraizer-ai/step-urdf-maker/releases/download/v0.1.5/step-urdf-maker-0.1.5-linux-x64.tar.gz)
- [Linux ARM64 다운로드 (.tar.gz)](https://github.com/kraizer-ai/step-urdf-maker/releases/download/v0.1.5/step-urdf-maker-0.1.5-linux-arm64.tar.gz)
